...
type UserRow = { id: number; name: string; email: string; role: "admin" | "staff"; outletId: number | null; outletName: string; permissions: string[] };
...
export function UserManagementClient({ users, currentUserId, outlets }: { users: UserRow[]; currentUserId: number; outlets: OutletOption[] }) {
  ...
  const checkedPermissions = editing?.permissions.length ? editing.permissions : DEFAULT_STAFF_PERMISSIONS;
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
            ...
            <form action={(formData) => startTransition(async () => { try { await upsertUser(formData); toast.success("User disimpan"); setOpen(false); setEditing(null); router.refresh(); } catch (error) { toast.error(error instanceof Error ? error.message : "Gagal menyimpan user"); } })} className="grid gap-4">
              ...
              {role === "staff" ? <div className="space-y-3 rounded-lg border border-slate-800 bg-slate-950/30 p-4"><p className="text-sm font-medium text-slate-100">Hak Akses Staff</p><div className="grid gap-4 sm:grid-cols-2">{permissionGroups.map((group) => <div key={group.group} className="space-y-2"><p className="text-xs font-semibold uppercase text-slate-500">{group.group}</p>{group.items.map((item) => <label key={item.key} className="flex items-center gap-2 text-sm text-slate-300"><input type="checkbox" name="permissions" value={item.key} defaultChecked={checkedPermissions.includes(item.key)} className="h-4 w-4 rounded border-slate-700 bg-slate-950" />{item.label}</label>)}</div>)}</div></div> : null}
