...
export function Topbar({ userName, role, outletName, activeOutletId, outlets, permissions }: { userName: string; role: Role; outletName: string; activeOutletId: number; selectedOutlet: number | "all" | null; outlets: Array<{ id: number; name: string }>; permissions: string[] }) {
  ...
  const outletControl = role === "admin" && !dashboardPage ? (
    ...
    <div className="flex h-9 items-center gap-2 rounded-md border border-slate-700 bg-slate-900/60 px-3 text-sm text-slate-300">
      <Building2 className="h-4 w-4" />
      <span>{role === "admin" && dashboardPage ? "Semua Cabang" : outletName}</span>
    </div>
  );
  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b border-slate-800/60 bg-slate-950/60 backdrop-blur-2xl">
        <div className="flex h-16 items-center justify-between gap-4 px-4 lg:px-8">
          <div className="flex min-w-0 items-center gap-3 lg:hidden">
            <Button variant="ghost" size="icon" type="button" aria-label="Buka menu" aria-expanded={mobileOpen} onClick={() => setMobileOpen(true)}><Menu className="h-5 w-5" /></Button>
            <div className="md:hidden"><span className="truncate font-semibold text-slate-100">PosPintar</span><span className="block text-[10px] text-slate-400">Manajemen</span></div>
          </div>
      ...
      <div className={cn("fixed inset-0 z-50 lg:hidden", mobileOpen ? "pointer-events-auto" : "pointer-events-none")} aria-hidden={!mobileOpen}>
        <button type="button" aria-label="Tutup menu" className={cn("absolute inset-0 bg-slate-950/70 transition-opacity", mobileOpen ? "opacity-100" : "opacity-0")} onClick={() => setMobileOpen(false)} />
        <aside className={cn("relative flex h-full w-[min(20rem,86vw)] flex-col border-r border-slate-800/60 bg-slate-950/80 backdrop-blur-2xl text-slate-100 shadow-2xl transition-transform duration-300 cubic-bezier(0.4, 0, 0.2, 1)", mobileOpen ? "translate-x-0" : "-translate-x-full")}>
          <div className="flex h-16 items-center justify-between gap-3 border-b border-slate-800/60 px-4">
            <div className="flex min-w-0 items-center"><div className="flex flex-col space-y-0.5 leading-none"><p className="truncate font-semibold text-slate-100">PosPintar</p><p className="truncate text-xs text-slate-400">Sistem Manajemen</p></div></div>
            <Button variant="ghost" size="icon" type="button" aria-label="Tutup menu" onClick={() => setMobileOpen(false)}><X className="h-5 w-5" /></Button>
          </div>
          <nav className="flex-1 space-y-1 overflow-y-auto p-4">
            {menuItems.map((item) => {
              const active = pathname === item.href || pathname.startsWith(`${item.href}/`);
              return <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={cn("flex items-center gap-3 rounded-r-lg px-3 py-3 text-sm font-medium transition-all duration-300", active ? "bg-gradient-to-r from-cyan-500/15 to-transparent text-white border-l-[3px] border-cyan-400 shadow-[inset_20px_0_40px_-20px_rgba(34,211,238,0.3)]" : "border-l-[3px] border-transparent text-slate-400 hover:bg-slate-800/40 hover:text-slate-100 hover:border-slate-700")}><item.icon className="h-4 w-4 shrink-0" /><span>{item.label}</span></Link>;
            })}
          </nav>
