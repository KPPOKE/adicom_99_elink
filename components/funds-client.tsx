...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return <label className="space-y-1.5"><span className="block text-xs font-semibold uppercase tracking-wide text-slate-600">{label}</span>{children}{hint ? <span className="block text-xs text-slate-500">{hint}</span> : null}</label>;
}
...
export function FundsClient({ funds }: { funds: FundRow[] }) {
  ...
  const columns: ColumnDef<FundRow>[] = [
    { header: "Sumber Dana", cell: ({ row }) => <div><p className="font-medium text-slate-900">{row.original.name}</p><p className="text-xs text-slate-500">{row.original.note || "-"}</p></div> },
    { header: "Tipe", cell: ({ row }) => <Badge variant="blue">{row.original.type.replace("_", " ")}</Badge> },
    { header: "Saldo", cell: ({ row }) => <span className="font-semibold text-blue-600">{formatCurrency(row.original.balance)}</span> },
  ...
  return <div className="space-y-6">
    <section className="grid gap-4 md:grid-cols-3">
      <div className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-3 text-slate-600"><Wallet className="h-5 w-5 text-blue-600" /><span className="text-sm">Total Aset</span></div><p className="mt-3 text-2xl font-semibold text-blue-600">{formatCurrency(totalCash + totalNonCash)}</p></div>
      <div className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-3 text-slate-600"><Wallet className="h-5 w-5 text-emerald-300" /><span className="text-sm">Aset Cash</span></div><p className="mt-3 text-2xl font-semibold text-blue-600">{formatCurrency(totalCash)}</p></div>
      <div className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-3 text-slate-600"><Landmark className="h-5 w-5 text-blue-300" /><span className="text-sm">Aset Non Tunai</span></div><p className="mt-3 text-2xl font-semibold text-blue-600">{formatCurrency(totalNonCash)}</p></div>
    </section>

    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-lg sm:p-6">
      <div className="mb-5"><h2 className="font-semibold text-slate-900">{form.id ? "Edit Sumber Dana" : "Tambah Sumber Dana"}</h2><p className="mt-1 text-sm text-slate-500">Saldo awal hanya saat membuat sumber dana baru. Perubahan saldo berikutnya lewat Mutasi Saldo.</p></div>
      <form action={() => run(() => upsertFundAccount(form), form.id ? "Sumber dana diperbarui" : "Sumber dana dibuat")} className="space-y-5">
        ...
        <div className="flex flex-col gap-2 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end"><Button type="submit" disabled={pending}>{form.id ? "Simpan Perubahan" : "Tambah Sumber Dana"}</Button>{form.id ? <Button type="button" variant="outline" onClick={() => setForm(empty)}>Batal Edit</Button> : null}</div>
      </form>
    </section>
