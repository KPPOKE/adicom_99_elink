...
export default async function FundsPage() {
  ...
  return <><PageHeader title="Saldo Awal" description={`Kelola LACI, bank, e-wallet, dan sumber dana cabang ${activeOutlet.name}.`} /><FundsClient canAdd={hasPermission(user.role.name, permissions, "funds.manage")} canEdit={hasPermission(user.role.name, permissions, "funds.edit")} funds={funds.map((item) => ({ id: item.id, name: item.name, image: item.image, type: item.type, balance: toNumber(item.balance), openingBalance: toNumber(item.openingBalance), note: item.note, isActive: item.isActive, updatedAt: item.updatedAt.toISOString() }))} /></>;
}
