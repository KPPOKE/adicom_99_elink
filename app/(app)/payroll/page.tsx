...
export default async function PayrollPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><PageHeader title="Penggajian Pegawai" description={`Penggajian cabang ${activeOutlet.name}.`} /><form className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><label className="space-y-1"><span className="text-xs font-medium text-slate-600">Bulan</span><Select name="month" defaultValue={String(month)} className="w-44">{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>{new Date(2026, index, 1).toLocaleDateString("id-ID", { month: "long" })}</option>)}</Select></label><label className="space-y-1"><span className="text-xs font-medium text-slate-600">Tahun</span><Select name="year" defaultValue={String(year)} className="w-32">{Array.from({ length: now.getFullYear() - 2019 }, (_, index) => now.getFullYear() - index).map((item) => <option key={item}>{item}</option>)}</Select></label><Button><Filter className="h-4 w-4" />Terapkan</Button></form><PayrollClient canManage={hasPermission(user.role.name, permissions, "payroll.manage")} employees={employees} rows={rows.map((item) => ({ id: item.id, employeeId: item.employeeId, employeeName: item.employee.name, period: item.period.toISOString(), baseSalary: toNumber(item.baseSalary), allowance: toNumber(item.allowance), deduction: toNumber(item.deduction), totalSalary: toNumber(item.totalSalary), paymentDate: item.paymentDate?.toISOString() ?? null, status: item.status, note: item.note }))} /></>;
}