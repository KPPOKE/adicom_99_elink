...
export default async function AccessSettingsPage() {
  ...
  return <><PageHeader title="Hak Akses Karyawan" description="Atur cabang dan checklist fitur setiap staff. Informasi laba dan keuntungan selalu khusus admin." /><Card><CardContent className="pt-6"><UserManagementClient currentUserId={currentUser.id} outlets={outlets} users={users.map((user) => ({ id: user.id, name: user.name, email: user.email, role: user.role.name, outletId: user.outletId, outletName: user.outlet?.name ?? "-", permissions: user.permissions.map((item) => item.key), isActive: user.isActive }))} /></CardContent></Card></>;
}